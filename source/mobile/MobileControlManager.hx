...
class MobileControlManager {
	...
	public function addMobilePadCamera(defaultDrawTarget:Bool = false, index:Int = -1):Void
	{
		...
		if (index == -1)
			...
			FlxG.cameras.insert(mobilePadCam, index, defaultDrawTarget);
	...
	public function addHitboxCamera(defaultDrawTarget:Bool = false, index:Int = -1):Void
	{
		...
		if (index == -1)
			...
			FlxG.cameras.insert(hitboxCam, index, defaultDrawTarget);
	...
	public function addJoyStickCamera(defaultDrawTarget:Bool = false, index:Int = -1):Void {
		joyStickCam = new FlxCamera();
		joyStickCam.bgColor.alpha = 0;
		if (index == -1)
			FlxG.cameras.add(joyStickCam, defaultDrawTarget);
		else
			FlxG.cameras.insert(joyStickCam, index, defaultDrawTarget);
