		...
		for (i in folders) {
			...
			switch (modes[intNumber]) {
				case ACTION:
					...
					#if MODS_ALLOWED
					for (folder in Util.directoriesWithFile(Paths.getPreloadPath(), 'mobile/MobilePad/')) {
				...
				case HITBOX:
					...
					#if MODS_ALLOWED
					for (folder in Util.directoriesWithFile(Paths.getPreloadPath(), 'mobile/Hitbox/')) {
...
{
	hints:Array<HitboxData>, //support old jsons (or just for global things)
